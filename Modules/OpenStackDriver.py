...
def OpenStack_main(slice):
    ...
    vlan=maxvlan+1
    if(len(id_s)==0):
        id_slice=conn.Insert("slice", "nombre,tipo,vlan_id,fecha_creacion,fecha_modificacion", f"'{nombre_slice}','openstack',{vlan},now(),now()")
    else:
        id_slice=id_s[0][0]
    ...
    for nodo_key in slice["nodos"]:
        ...
        if(nodo["instanciado"]=="false"):
            ...
            if nodo["config"]["type"] == "manual":
                ...
                id_flavor=create_flavor(TOKEN,str(nombre_flavor),int(recursos[1]),int(recursos[0]),int(recursos[2]))
...
def guardarRecursos(nombre,cpu, ram, storage, ramUtil,storageU, vcpu,vcpuUtil):
    ...
    if(len(tupla)!=0):
        ...
        conexion.Update('recursos',f'ram_available="{ramUtil}",storage_available="{storageU}"',f'id_recursos={int(id_recurso_encontrado)}')
